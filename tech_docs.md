...
- [6. Project Directory Structure (Node.js Backend)](#6-project-directory-structure-nodejs-backend)
...
Two workflows are supported for local development:

1. **Full-containerized development** (recommended) — `docker-compose.dev.yml` runs *everything* in Docker with live hot-reloading via bind mounts.
2. **Non-Docker development** — run only the foundational services (Postgres, Redis, Immich ML) in Docker and run the Fastify backend + SvelteKit frontend directly on your host for the fastest possible HMR.

### Prerequisites
- **Node.js**: v18 or newer (v22 recommended)
- **Docker** & **Docker Compose**: Installed and running

### 7.1 Setup Local Volumes
Before starting, place some test photos in your local media folder so the scanner has something to read. Both workflows mount `./volumes/media_ro` (read-only) into the containers.
...
### 7.2 Option A — Full Containerized Stack (Hot-Reload)
The easiest way to develop. `docker-compose.dev.yml` bind-mounts your local `./frontend` and `./backend` directories into the containers, so any file save instantly hot-reloads (Vite HMR for the frontend, `tsx watch` for the backend).

```bash
cp .env.example .env
docker compose -f docker-compose.dev.yml up -d --build
```
*Wait a few seconds for PostgreSQL to initialize the schema via the `pg_init/*.sql` scripts.*

| Service | URL |
| --- | --- |
| Frontend UI (Vite dev server) | http://localhost:8000 |
| Backend API | internal — proxied by the frontend via `BACKEND_PROXY_URL=http://backend:3000` |
| Redis | http://localhost:8003 |
| Immich ML | http://localhost:8004 |

### 7.3 Option B — Non-Docker Local Development (Fastest HMR)
Run Postgres, Redis, and Immich ML in Docker, and the Node processes directly on your host.

First, start the three foundational services with published host ports so the host processes can reach them:

```bash
# Postgres with pgvector + init scripts
docker run -d --name hmp-pg \
  -e POSTGRES_USER=hellomyphotos -e POSTGRES_PASSWORD=hellomyphotos_secret -e POSTGRES_DB=hellomyphotos \
  -p 5432:5432 \
  -v "$PWD/volumes/pg_init:/docker-entrypoint-initdb.d:ro" \
  ankane/pgvector:v0.5.1

# Redis
docker run -d --name hmp-redis -p 6379:6379 redis:7-alpine

# Immich ML
docker run -d --name hmp-ml -p 3003:3003 ghcr.io/immich-app/immich-machine-learning:release
```

Create a `backend/.env.local` so the host backend points at those published ports:
...
Then install and run both apps:
...
The frontend dev proxy (`vite.config.ts`) forwards `/api/*` to `BACKEND_PROXY_URL || http://localhost:3000`.

### 7.4 Testing the Flow
1. Open `http://localhost:5173/folder/test_folder` in your browser (or `http://localhost:8000` if using the full-container stack).
2. Check the backend output: you should see `ScannerService` indexing the files and pushing jobs to the BullMQ workers.
3. Check `volumes/cache_rw/`: you should see `1080p` and `480p` WebP thumbnails appearing.
4. Navigate to the **Faces** tab in the UI to see clustered facial recognition results generated by the Immich ML container.
...
## 10. Docker Compose Production Runbook

This runbook covers operating the **production** `docker-compose.yml` stack. In production only three host ports are exposed by default:

| Service | Host port | Purpose |
| --- | --- | --- |
| `frontend` | `8000` | The SvelteKit UI, which also reverse-proxies `/api/*` to the backend |
| `redis` | `8003` | Diagnostics/debugging only — do not expose publicly |
| `machine-learning` | `8004` | Diagnostics/debugging only — do not expose publicly |

`backend`, `backend-worker`, and `postgres` are **not** exposed to the host; they communicate only over the internal Compose network.

### 10.1 First Boot

```bash
cp .env.example .env
# Edit .env: set APP_DOMAIN, generate strong DB_PASS / POSTGRES_PASSWORD,
# add JWT_SECRET and COOKIE_SECRET (not present in .env.example).
mkdir -p volumes/media_ro volumes/cache_rw
docker compose up -d --build
```

On first boot, PostgreSQL initializes the schema from `./volumes/pg_init/*.sql` (these scripts only run on an **empty** `pg_data` volume). Wait ~30 seconds, then open `http://localhost:8000`.

### 10.2 Default Credentials

The seed script creates an admin account on first boot:
...
Change this password immediately after first login, or update the seed before first boot.

### 10.3 Scanning Media

The stack reads media **read-only** from `./volumes/media_ro` (mounted `:ro`). Drop `.jpg`, `.png`, or `.mp4` files into any subfolder (e.g. `./volumes/media_ro/holiday_2024/`) and open `/folder/holiday_2024` in the UI to trigger an on-demand scan. Cached 1080p/480p WebP thumbnails and face embeddings are written to `./volumes/cache_rw`.
...
Pull new images and recreate changed containers. Persisted state lives in the `./volumes` bind mounts, so nothing is lost.
...
**Warning:** `-v` deletes the named volumes **and** the local bind-mount data tied to Compose cleanup. `./volumes/media_ro` (your originals) is preserved, but `pg_data`, `cache_rw`, `redis_data`, and `ml_cache` are wiped. After a reset, `pg_init` re-seeds the schema and `cache_rw` is rebuilt on the next scan. Back up `pg_data` first if you care about existing metadata/analytics.

### 10.6 Backup & Restore

Back up the database (the only irreplaceable state besides `media_ro`):

```bash
docker compose exec postgres pg_dump -U hellomyphotos hellomyphotos > backup.sql
```

Restore:

```bash
cat backup.sql | docker compose exec -T postgres psql -U hellomyphotos hellomyphotos
```

`cache_rw`, `redis_data`, and `ml_cache` do **not** need backing up — they are regenerated automatically (cache on next scan, embeddings via ML re-index, Redis analytics flush to Postgres).

### 10.7 Reverse Proxy & TLS

Terminate TLS on a host reverse proxy (Caddy, Nginx, Traefik) and proxy only the `frontend` port:

- Proxy `http://localhost:8000` (HTTP) or `https://domain` → `http://127.0.0.1:8000`.
- Ensure `Range` headers pass through so HTTP video streaming works on Safari.
- Set `APP_DOMAIN` in `.env` to your public `https://` domain so share links and redirects are absolute.

### 10.8 Security Checklist
- [ ] Generate strong `DB_PASS` / `POSTGRES_PASSWORD` and never commit `.env`.
- [ ] Add `JWT_SECRET` and `COOKIE_SECRET` (long random strings) to `.env`.
